...
Deno.test("api/v1/accounts test should fail if the query parameters are malformed", async () => {
  ...
  let res = await AuthenticatedGET({
    url: "http://localhost:3000/api/v1/accounts?current_page=asd",
    accesstoken,
  });
  ...
  res = await AuthenticatedGET({
    url: "http://localhost:3000/api/v1/accounts?page_size=asd",
    accesstoken,
  });
  assertEquals(res.status, 400);
  json = await res.json();
  ...
  res = await AuthenticatedGET({
    url: "http://localhost:3000/api/v1/accounts?sort_direction=asd",
    accesstoken,
  });
  assertEquals(res.status, 400);
  json = await res.json();
  ...
  res = await AuthenticatedGET({
    url: "http://localhost:3000/api/v1/accounts?sort_by=asd",
    accesstoken,
  });
  assertEquals(res.status, 400);
  json = await res.json();
...
Deno.test("api/v1/accounts should filter!", async () => {
  ...
  const res = await AuthenticatedGET({
    url: "http://localhost:3000/api/v1/accounts?filter=" + filter,
    accesstoken,
  });
