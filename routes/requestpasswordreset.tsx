...
import { sendPasswordResetEmail } from "../lib/backend/db/auth.ts";
...
export const handler: Handlers<any, State> = {
    ...
    async POST(req, ctx) {
        ...
        const { error } = await sendPasswordResetEmail(ctx.state.supabaseClient, email,);
        const headers = new Headers();

        let redirect = `/requestpasswordreset?success=${`Email sent! Check your mailbox.`}`

        if (error) {
            redirect = `/requestpasswordreset?error=${error.message}`
        }
                    ...
                    <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
                        ...
                        <div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0 bg-gradient-gray-to-white-variant2">
                            <div class="mx-auto pt-1 pb-1">
                                <h2 class="text-2xl text-center text-gray-600">Request password reset</h2>
                                <p class="text-lg text-center">We will send a custom link to your email address that will allow you to reset your password!</p>
                            </div>
                            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                                ...
                                <form class="space-y-4 md:space-y-6" method="POST">
                                    ...
                                    <button type="submit" class="w-full text-white bg-indigo-500 hover:bg-indigo-600 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-2xl px-5 py-2.5 text-center dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800">Resend</button>
                                    <p class="text-lg ">
                                        Go to <a href="/login" class="font-medium text-indigo-600 hover:underline dark:text-indigo-500">Login</a>
                                    </p>
                                    <hr />
                                </form>
                            </div>
