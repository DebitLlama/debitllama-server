...
import { resendEmailConfirmation } from "../lib/backend/db/auth.ts";
...
export const handler: Handlers<any, State> = {
    ...
    async POST(req, ctx) {
        ...
        const { error } = await resendEmailConfirmation(ctx.state.supabaseClient, email,);
        const headers = new Headers();

        let redirect = `/resendConfirmation?success=${`Email sent! Check your mailbox.`}`

        if (error) {
            redirect = `/resendConfirmation?error=${error.message}`
        }
                    ...
                    <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
                        ...
                        <div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0 bg-gradient-gray-to-white-variant2">
                            <div class="mx-auto pt-1 pb-1">
                                <h2 class="text-2xl text-center text-gray-600">Resend Email Confirmation</h2>
                            </div>
