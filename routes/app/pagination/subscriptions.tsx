...
export const handler: Handlers<any, State> = {
    async POST(_req, ctx) {
        ...
        if (searchTerm === "") {
            const { data: piRows, count } = await select.PaymentIntents.allByUserIdForCreatorPaginated(
                order,
                sortDirection === "ASC",
                from,
                to
            ...
            const { data: piRows, count } = await select.PaymentIntents.allByUserIdForCreatorPaginatedWithSearch(
                order,
                sortDirection === "ASC",
                from,
                to,
