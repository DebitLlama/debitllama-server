...
export default function PaymentIntentsPaginationForAccounts(
  props: PaymentIntentsPaginationForAccountsProps,
) {
  ...
  const fetchData = async (currentPage: number, searchTerm: string) => {
    const res = await fetchPaginatedPaymentIntentsForAccount({
